...
#define TAG_LCD "I2C_LCD_PCF8574"
...
void lcd_print_number(i2c_lcd_pcf8574_handle_t* lcd, uint8_t col, uint8_t row, uint8_t buf_len, const char *str, ...) {
    ...
    if (buf_len == 0)
    {
        ESP_LOGE(TAG_LCD, "Buffer length must be greater than 0");
    }
    ...
    if (chars_written < 0) {
        ESP_LOGE(TAG_LCD, "Encoding error in vsnprintf");
    }

    if ((size_t)chars_written >= buf_len) {
        ESP_LOGW(TAG_LCD, "Buffer overflow: %d characters needed, but only %d available", chars_written + 1, buf_len);
    }
...
static void lcd_send(i2c_lcd_pcf8574_handle_t* lcd, uint8_t value, bool is_data) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_LCD, "Failed to send data to LCD: %s", esp_err_to_name(ret));
    }
}  // lcd_send()
...
static void lcd_write_i2c(i2c_lcd_pcf8574_handle_t* lcd, uint8_t data, bool is_data, bool enable) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_LCD, "Failed to write to LCD: %s", esp_err_to_name(ret));
    }
}  // lcd_write_i2c()